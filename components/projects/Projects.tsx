...
const categoryColors: Record<string, string> = {
  'Web App': '#5B7C88',
  'AI Chatbot': '#8FBAC7',
  'CRM Dashboard': '#B3E1E8',
  'Android App': '#5B7C88',
  'SaaS Platform': '#8FBAC7',
  'Business Automation': '#B3E1E8',
};
...
export default function Projects() {
  ...
  return (
    <section className="relative py-28 overflow-hidden" style={{ background: '#131313' }}>

      {/* BG glow */}
      <div className="absolute bottom-0 right-0 w-[600px] h-[400px] rounded-full opacity-8 pointer-events-none"
        style={{ background: 'radial-gradient(ellipse, rgba(179,225,232,0.15) 0%, transparent 70%)', filter: 'blur(80px)' }} />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-5"
            ...
            <span className="text-[12px] font-semibold tracking-wide uppercase" style={{ color: '#B3E1E8' }}>Our Work</span>
          </div>
          <h2 className="text-[40px] md:text-[52px] font-black text-white tracking-tight mb-5">
            Real Products.{' '}
            <span style={{ background: 'linear-gradient(135deg,#B3E1E8,#5B7C88)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              Real Impact.
            </span>
          </h2>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-12">
          {filters.map(f => (
            <button key={f} onClick={() => setActiveFilter(f)}
              ...
              style={{
                background: activeFilter === f ? 'rgba(91,124,136,0.2)' : 'rgba(255,255,255,0.03)',
                border: activeFilter === f ? '1px solid rgba(179,225,232,0.4)' : '1px solid rgba(255,255,255,0.08)',
                color: activeFilter === f ? '#B3E1E8' : 'rgba(255,255,255,0.4)',
              }}>
              {f}
            </button>
          ))}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((project, i) => (
            <div key={i} className="group rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1"
              style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(179,225,232,0.08)', backdropFilter: 'blur(12px)' }}>

              {/* App mockup preview */}
              <div className="relative overflow-hidden" style={{ background: 'rgba(0,0,0,0.4)', borderBottom: '1px solid rgba(179,225,232,0.08)' }}>
                {/* Browser bar */}
                ...
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div className="ml-auto px-2 py-0.5 rounded-full text-[10px] font-medium"
                      style={{ background: 'rgba(179,225,232,0.1)', color: '#B3E1E8', border: '1px solid rgba(179,225,232,0.2)' }}>
                      Live
                    </div>
                  </div>
                  ...
                  <div className="flex gap-1 mb-3">
                    {project.screens.map((s, si) => (
                      <div key={si} className="px-2 py-0.5 rounded text-[10px]"
                        style={{ background: si === 0 ? 'rgba(91,124,136,0.2)' : 'rgba(255,255,255,0.04)', color: si === 0 ? '#B3E1E8' : 'rgba(255,255,255,0.3)' }}>
                        {s.label}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex items-end gap-1 h-10 mb-3">
                    {project.screens[0].bars.map((h, bi) => (
                      <div key={bi} className="flex-1 rounded-sm transition-all duration-300"
                        style={{ height: `${h}%`, background: bi === project.screens[0].bars.length - 1 ? `linear-gradient(to top, ${project.accent}, #B3E1E8)` : `rgba(91,124,136,0.25)` }} />
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {['Performance', 'Uptime', 'Users'].map((m, mi) => (
                      <div key={mi} className="rounded p-1.5 text-center" style={{ background: 'rgba(91,124,136,0.06)', border: '1px solid rgba(179,225,232,0.06)' }}>
                        <div className="text-[10px] text-white/25">{m}</div>
                        <div className="text-[11px] font-bold" style={{ color: '#B3E1E8' }}>{['98%', '99.9%', '↑ 3x'][mi]}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="p-5">
                ...
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-[11px] px-2.5 py-1 rounded-full font-medium"
                    style={{ background: 'rgba(91,124,136,0.1)', border: '1px solid rgba(179,225,232,0.15)', color: '#B3E1E8' }}>
                    {project.category}
                  </span>
                ...
                {/* Title */}
                <h3 className="text-[16px] font-bold text-white mb-2 group-hover:text-[#B3E1E8] transition-colors duration-200">
                  {project.title}
                </h3>
        ...
        <div className="text-center mt-14">
          <Link href="/contact"
            ...
            style={{ background: 'linear-gradient(135deg,#5B7C88,#8FBAC7)', boxShadow: '0 8px 32px rgba(91,124,136,0.4)' }}>
            Start Your Project
