...
export default function Solutions() {
  ...
  return (
    <section ref={sectionRef} className="relative py-28 overflow-hidden" style={{ background: '#131313' }}>
      <div className="absolute top-0 left-0 right-0 h-px" style={{ background: 'linear-gradient(90deg,transparent,rgba(91,124,136,0.35),transparent)' }} />
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-14 gap-6">
          <div className="reveal">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[12px] font-semibold text-[#B3E1E8] mb-5" style={{ background: 'rgba(91,124,136,0.1)', border: '1px solid rgba(179,225,232,0.2)' }}>
              OUR SOLUTIONS
            </div>
            <h2 className="text-[40px] sm:text-[52px] font-black tracking-tight text-white leading-tight">
              Every automation<br />
              <span style={{ background: 'linear-gradient(135deg,#B3E1E8,#5B7C88)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' }}>you'll ever need.</span>
            </h2>
          </div>
          <Link href="/solutions" className="reveal inline-flex items-center gap-2 text-[14px] font-semibold text-[#B3E1E8] hover:text-[#B3E1E8] transition-colors self-start lg:self-end">
            View all solutions
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <Link href={solutions[0].href} className="reveal lg:col-span-1 lg:row-span-2 group relative rounded-2xl p-8 flex flex-col justify-between min-h-[280px] transition-all duration-300 hover:-translate-y-1"
            style={{ background: 'rgba(91,124,136,0.08)', border: '1px solid rgba(179,225,232,0.2)' }}
            onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background='rgba(91,124,136,0.13)'; (e.currentTarget as HTMLAnchorElement).style.boxShadow='0 20px 60px rgba(179,225,232,0.2)'; }}
            onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background='rgba(91,124,136,0.08)'; (e.currentTarget as HTMLAnchorElement).style.boxShadow='none'; }}
          >
            <div>
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-6 text-[#B3E1E8]" style={{ background: 'rgba(179,225,232,0.2)', border: '1px solid rgba(179,225,232,0.25)' }}>{solutions[0].icon}</div>
              <div className="inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold text-[#B3E1E8] mb-4" style={{ background: 'rgba(91,124,136,0.12)' }}>FEATURED</div>
            ...
            <div className="flex items-center justify-between mt-6">
              <div className="flex gap-2">{solutions[0].tags.map(t => <span key={t} className="px-2.5 py-1 rounded-lg text-[11px] font-medium text-[#B3E1E8]" style={{ background: 'rgba(91,124,136,0.1)' }}>{t}</span>)}</div>
              <svg className="w-5 h-5 text-[#B3E1E8] group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
            </div>
          </Link>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-px" style={{ background: 'linear-gradient(90deg,transparent,rgba(91,124,136,0.15),transparent)' }} />
    </section>
  );
}
