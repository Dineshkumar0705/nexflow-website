...
export default function FinalCTAV2() {
  ...
  return (
    <section ref={sectionRef} className="relative py-28 overflow-hidden" style={{ background: '#131313' }}>
      <div className="absolute top-0 left-0 right-0 h-px" style={{ background: 'linear-gradient(90deg,transparent,rgba(91,124,136,0.4),transparent)' }} />

      {/* Background glow */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute inset-0 opacity-25" style={{ background: 'radial-gradient(ellipse 70% 60% at 50% 50%, rgba(91,124,136,0.28) 0%, transparent 70%)' }} />
      </div>
      <div className="absolute inset-0 grid-overlay opacity-20 pointer-events-none" />

      <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
        <div className="reveal inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[12px] font-semibold text-[#B3E1E8] mb-8" style={{ background: 'rgba(91,124,136,0.1)', border: '1px solid rgba(179,225,232,0.2)' }}>
          <span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#5B7C88] opacity-75"/><span className="relative inline-flex rounded-full h-2 w-2 bg-[#5B7C88]"/></span>
          Ready when you are
        </div>
        ...
        <h2 className="reveal text-[44px] sm:text-[60px] lg:text-[72px] font-black tracking-tight leading-tight mb-6">
          ...
          <span style={{ background: 'linear-gradient(135deg,#B3E1E8 0%,#8FBAC7 50%,#B3E1E8 100%)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text', backgroundSize:'200% 200%', animation:'gradient-x 4s ease infinite' }}>
            work that machines
          </span><br/>
        ...
        <div className="reveal flex flex-col sm:flex-row items-center justify-center gap-4 mb-10">
          <Link
            ...
            style={{ background: 'linear-gradient(135deg,#5B7C88,#8FBAC7)', boxShadow: '0 8px 40px rgba(91,124,136,0.3), 0 0 0 1px rgba(179,225,232,0.25)' }}
          >
        ...
        <div className="reveal flex items-center justify-center gap-6 flex-wrap text-[13px] text-white/30">
          {['Free 30-min call', 'No credit card', 'Results in days', 'Cancel anytime'].map((item, i) => (
            <span key={i} className="flex items-center gap-1.5">
              <svg className="w-3.5 h-3.5 text-[#5B7C88]" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
              {item}
            </span>
          ))}
        </div>
