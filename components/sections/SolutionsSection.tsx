    ...
    <section className="relative py-28 overflow-hidden" style={{ background: '#131313' }}>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] rounded-full opacity-10"
          style={{ background: 'radial-gradient(ellipse, rgba(91,124,136,0.3) 0%, transparent 70%)', filter: 'blur(80px)' }} />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-5"
            ...
            <span className="text-[12px] font-semibold tracking-wide uppercase" style={{ color: '#B3E1E8' }}>What We Build</span>
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {solutions.map((item, i) => (
            <Link href={item.href} key={i} className="group block">
              <div className="relative rounded-2xl p-6 h-full transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-[0_20px_60px_rgba(91,124,136,0.15)]"
                ...
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl mb-5"
                  style={{ background: 'rgba(91,124,136,0.1)', border: '1px solid rgba(179,225,232,0.12)' }}>
                  {item.icon}
                </div>

                {/* Title */}
                <h3 className="text-[17px] font-bold text-white mb-3 group-hover:text-[#B3E1E8] transition-colors duration-200">
                  {item.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {item.tags.map((tag, t) => (
                    <span key={t} className="text-[11px] px-2.5 py-1 rounded-full"
                      style={{ background: 'rgba(91,124,136,0.1)', border: '1px solid rgba(179,225,232,0.12)', color: 'rgba(179,225,232,0.3)' }}>
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-1.5 text-[13px] font-semibold transition-colors duration-200"
                  style={{ color: 'rgba(179,225,232,0.28)' }}>
                  <span className="group-hover:text-[#B3E1E8] transition-colors">Explore →</span>
                </div>
              </div>
            </Link>
          ))}
