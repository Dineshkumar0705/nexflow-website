...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center overflow-hidden" style={{ background: '#131313' }}>

      {/* Gradient orbs */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute -top-[20%] -left-[10%] w-[700px] h-[700px] rounded-full opacity-20"
          style={{ background: 'radial-gradient(circle, rgba(91,124,136,0.3) 0%, transparent 70%)', filter: 'blur(80px)', animation: 'mesh-move 18s ease-in-out infinite' }} />
        <div className="absolute top-[30%] -right-[10%] w-[600px] h-[600px] rounded-full opacity-15"
          style={{ background: 'radial-gradient(circle, rgba(179,225,232,0.28) 0%, transparent 70%)', filter: 'blur(90px)', animation: 'mesh-move-2 22s ease-in-out infinite' }} />
        <div className="absolute bottom-0 left-[30%] w-[400px] h-[300px] rounded-full opacity-10"
          style={{ background: 'radial-gradient(circle, rgba(143,186,199,0.3) 0%, transparent 70%)', filter: 'blur(60px)' }} />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-32 pb-20">
        <div className={`text-center max-w-4xl mx-auto transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full mb-8 transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
            style={{ background: 'rgba(91,124,136,0.12)', border: '1px solid rgba(179,225,232,0.25)' }}>
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75" style={{ background: '#B3E1E8' }} />
              <span className="relative inline-flex rounded-full h-2 w-2" style={{ background: '#5B7C88' }} />
            </span>
            <span className="text-[13px] font-medium" style={{ color: '#B3E1E8' }}>AI · SaaS · IT Services — All Under One Roof</span>
          </div>
          ...
          <h1 className={`text-[52px] sm:text-[64px] lg:text-[80px] font-black tracking-tight leading-[1.06] mb-6 transition-all duration-700 delay-100 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            ...
            <br />
            <span style={{ background: 'linear-gradient(135deg, #B3E1E8 0%, #5B7C88 50%, #8FBAC7 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', backgroundSize: '200% 200%', animation: 'gradient-x 4s ease infinite' }}>
              Ship Faster.
            </span>
            <br />
            <span className="text-white">Scale</span>{' '}
            <span style={{ background: 'linear-gradient(135deg, #8FBAC7, #B3E1E8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Further.</span>
          </h1>
          ...
          <div className={`flex flex-col sm:flex-row items-center justify-center gap-4 mb-14 transition-all duration-700 delay-300 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <Link
              ...
              style={{ background: 'linear-gradient(135deg,#5B7C88,#8FBAC7)', boxShadow: '0 8px 32px rgba(91,124,136,0.28), 0 0 0 1px rgba(179,225,232,0.2)' }}
            >
              Start Your Project
            ...
            <Link
              ...
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(179,225,232,0.15)', backdropFilter: 'blur(8px)' }}
            >
          ...
          <div className={`flex items-center justify-center gap-3 flex-wrap mb-16 transition-all duration-700 delay-400 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
            {pillTags.map((tag, i) => (
              <span key={i} className="flex items-center gap-1.5 text-[12px] px-3 py-1 rounded-full"
                style={{ background: 'rgba(91,124,136,0.1)', border: '1px solid rgba(179,225,232,0.15)', color: 'rgba(179,225,232,0.3)' }}>
        ...
        <div className={`relative max-w-5xl mx-auto transition-all duration-1000 delay-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
          <div className="absolute -inset-8 rounded-3xl opacity-25 blur-3xl" style={{ background: 'linear-gradient(135deg,rgba(91,124,136,0.28),rgba(179,225,232,0.2))' }} />

          <div className="relative rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(179,225,232,0.1)', backdropFilter: 'blur(16px)' }}>
            {/* Browser chrome */}
            ...
            <div className="p-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              {[
                ...
                <div key={i} className="rounded-xl p-4" style={{ background: 'rgba(91,124,136,0.06)', border: '1px solid rgba(179,225,232,0.1)' }}>
                  ...
                  <div className="h-1.5 rounded-full" style={{ background: 'rgba(179,225,232,0.1)' }}>
                    <div className="h-1.5 rounded-full animate-pulse" style={{ width: `${svc.pct}%`, background: 'linear-gradient(90deg,#5B7C88,#B3E1E8)' }} />
                  </div>
                </div>
              ))}

              <div className="sm:col-span-2 rounded-xl p-5" style={{ background: 'rgba(91,124,136,0.04)', border: '1px solid rgba(179,225,232,0.08)' }}>
                  ...
                  {['Scope', 'Design', 'Build', 'QA Test', 'Deploy', 'Live ✓'].map((step, i) => (
                    <div key={i} className="flex items-center gap-1.5">
                      <div className="px-3 py-1.5 rounded-lg text-[11px] font-medium" style={{
                        background: i === 5 ? 'rgba(179,225,232,0.15)' : 'rgba(91,124,136,0.15)',
                        border: `1px solid ${i === 5 ? 'rgba(179,225,232,0.4)' : 'rgba(91,124,136,0.3)'}`,
                        color: i === 5 ? '#B3E1E8' : '#8FBAC7'
                      }}>{step}</div>
                ...
                <div className="flex items-end gap-1.5 h-12">
                  {[45,60,40,75,55,85,65,90,70,88,80,95].map((h, i) => (
                    <div key={i} className="flex-1 rounded-sm" style={{ height: `${h}%`, background: i === 11 ? 'linear-gradient(to top,#5B7C88,#B3E1E8)' : 'rgba(91,124,136,0.2)' }} />
                  ))}
                </div>
              </div>

              <div className="sm:col-span-1 rounded-xl p-4 space-y-2" style={{ background: 'rgba(91,124,136,0.04)', border: '1px solid rgba(179,225,232,0.08)' }}>
                ...
                {[
                  ...
                  <div key={i} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: '#B3E1E8' }} />
                      <span className="text-[11px] text-white/50 truncate">{item.label}</span>
                    </div>
                    <span className="text-[10px] px-1.5 py-0.5 rounded" style={{ background: 'rgba(179,225,232,0.08)', color: 'rgba(179,225,232,0.28)' }}>{item.type}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-32 pointer-events-none" style={{ background: 'linear-gradient(to top, #131313, transparent)' }} />
    </section>
  );
}
