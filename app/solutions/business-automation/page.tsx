...
const workflowSteps = [
  { icon: '📥', label: 'Lead submits form', color: '#5B7C88' },
  { icon: '🔍', label: 'AI scores & qualifies', color: '#5B7C88' },
  { icon: '💾', label: 'CRM auto-updated', color: '#8FBAC7' },
  { icon: '📱', label: 'WhatsApp follow-up sent', color: '#B3E1E8' },
  { icon: '📅', label: 'Meeting auto-booked', color: '#B3E1E8' },
];

const tools = ['n8n', 'Make.com', 'Zapier', 'OpenAI', 'WhatsApp Business API', 'HubSpot', 'Airtable', 'Google Workspace', 'Slack', 'Webhooks'];

const G = '#5B7C88', M = '#B3E1E8', BG = '#131313';
      ...
      <main style={{ background: BG, minHeight: '100vh', color: '#fff' }}>
        ...
        <section className="relative pt-40 pb-28 px-6 overflow-hidden">
          ...
          <div className="relative z-10 max-w-4xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[12px] font-bold mb-6 uppercase tracking-widest" style={{ background: 'rgba(91,124,136,0.1)', border: '1px solid rgba(179,225,232,0.25)', color: M }}>
              Business Automation
            </div>
                ...
                <div key={i} className="flex items-center gap-3">
                  <div className="flex flex-col items-center">
                    <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl mb-2"
                      style={{ background: `rgba(91,124,136,0.15)`, border: `1px solid rgba(179,225,232,0.2)` }}>
                      {step.icon}
                    </div>
                    <span className="text-[11px] text-white/40 text-center max-w-[80px]">{step.label}</span>
                  </div>
                  {i < workflowSteps.length - 1 && (
                    <svg className="w-5 h-5 hidden sm:block mb-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} style={{ color: 'rgba(179,225,232,0.3)' }}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/contact" className="inline-flex items-center gap-2.5 px-8 py-4 rounded-2xl text-[16px] font-bold text-white transition-all hover:-translate-y-1"
                style={{ background: `linear-gradient(135deg,${G},#8FBAC7)`, boxShadow: `0 8px 32px rgba(91,124,136,0.28)` }}>
              ...
              <Link href="/projects" className="inline-flex items-center gap-2.5 px-8 py-4 rounded-2xl text-[15px] font-semibold text-white/70 hover:text-white transition-all"
                style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(179,225,232,0.15)' }}>
                See Case Studies
              </Link>
            </div>
        ...
        {/* FEATURES */}
        <section className="py-24 px-6" style={{ borderTop: '1px solid rgba(179,225,232,0.08)' }}>
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-14">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-5 text-[11px] font-bold uppercase tracking-widest" style={{ background: 'rgba(91,124,136,0.1)', border: '1px solid rgba(179,225,232,0.2)', color: M }}>What We Automate</div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {features.map((f, i) => (
                <div key={i} className="group rounded-2xl p-7 transition-all duration-300 hover:-translate-y-1"
                  style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(179,225,232,0.08)' }}>
                  <div className="text-3xl mb-4">{f.icon}</div>
                  <h3 className="text-[18px] font-bold text-white mb-2 group-hover:text-[#B3E1E8] transition-colors">{f.title}</h3>
        ...
        {/* TOOLS */}
        <section className="py-20 px-6" style={{ borderTop: '1px solid rgba(179,225,232,0.08)' }}>
          <div className="max-w-4xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-5 text-[11px] font-bold uppercase tracking-widest" style={{ background: 'rgba(91,124,136,0.1)', border: '1px solid rgba(179,225,232,0.2)', color: M }}>Tools & Platforms</div>
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {tools.map((t, i) => (
                <span key={i} className="px-5 py-2.5 rounded-xl text-[14px] font-semibold transition-all hover:-translate-y-0.5"
                  style={{ background: 'rgba(91,124,136,0.08)', border: '1px solid rgba(179,225,232,0.15)', color: 'rgba(179,225,232,0.35)' }}>
                  {t}
                </span>
              ))}
            </div>
        ...
        {/* CTA */}
        <section className="py-24 px-6 text-center" style={{ borderTop: '1px solid rgba(179,225,232,0.08)' }}>
          ...
          <Link href="/contact" className="inline-flex items-center gap-2.5 px-10 py-5 rounded-2xl text-[17px] font-bold text-white transition-all hover:-translate-y-1"
            style={{ background: `linear-gradient(135deg,${G},#8FBAC7)`, boxShadow: `0 8px 40px rgba(91,124,136,0.28)` }}>
            Get My Free Audit →
          </Link>
        </section>
      </main>
