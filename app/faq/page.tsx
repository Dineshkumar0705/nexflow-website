...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden transition-all duration-200" style={{ background: open ? 'rgba(91,124,136,0.06)' : 'rgba(255,255,255,0.025)', border: open ? '1px solid rgba(91,124,136,0.15)' : '1px solid rgba(255,255,255,0.07)' }}>
      <button className="w-full flex items-center justify-between gap-4 px-6 py-5 text-left" onClick={() => setOpen(!open)}>
        ...
        <svg className={`w-5 h-5 text-[#B3E1E8] flex-shrink-0 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7"/></svg>
      </button>
    ...
    <>
      ...
      <main style={{ background: '#131313', minHeight: '100vh' }}>
        <section className="relative pt-40 pb-20 px-6 overflow-hidden">
          <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 60% 50% at 50% 0%, rgba(91,124,136,0.1) 0%, transparent 70%)' }} />
          <div className="relative z-10 max-w-3xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[12px] font-semibold text-[#B3E1E8] mb-6" style={{ background: 'rgba(91,124,136,0.1)', border: '1px solid rgba(179,225,232,0.2)' }}>FAQ</div>
            <h1 className="text-[52px] sm:text-[64px] font-black tracking-tight text-white leading-tight mb-5">
              Frequently Asked<br />
              <span style={{ background: 'linear-gradient(135deg,#B3E1E8,#5B7C88)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' }}>Questions</span>
            </h1>
        ...
        <section className="pb-28 px-6" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          <div className="max-w-3xl mx-auto mt-12 text-center">
            ...
            <Link href="/contact" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-[14px] font-semibold text-white transition-all hover:-translate-y-0.5" style={{ background: 'linear-gradient(135deg,#5B7C88,#8FBAC7)', boxShadow: '0 4px 20px rgba(91,124,136,0.35)' }}>
              Contact Us →
            </Link>
          </div>
        </section>
